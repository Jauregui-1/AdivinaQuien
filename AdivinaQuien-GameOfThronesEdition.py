...
import os  # Para operaciones del sistema como verificar existencia de archivos
import math  # Para cálculos matemáticos (logaritmos)
from tkinter import *  # Para la interfaz gráfica
from tkinter import messagebox, simpledialog, ttk  # Componentes específicos de tkinter
from typing import Dict, List, Optional, Tuple, Union  # Para tipado estático


# Definición de la clase Nodo que representa cada nodo del árbol de decisión
class Nodo:
    def __init__(self, atributo: str = None, valor: str = None, personaje: Dict = None):
        self.atributo = atributo  # Atributo que se pregunta en este nodo (ej: "casa")
        self.valor = valor  # Valor específico del atributo (ej: "Stark")
        self.personaje = personaje  # Personaje almacenado (solo en nodos hoja)
        self.si = None  # Rama para respuesta afirmativa
        self.no = None  # Rama para respuesta negativa

# Clase principal del juego
class AdivinaQuienGoT:
    # Atributos disponibles para las preguntas
    ATRIBUTOS_DISPONIBLES = ["género", "casa", "vivo", "titulo", "lugar", "muerte"]
    ...
    def __init__(self, root: Tk):
        ...
        self.root.title("Adivina Quién - Game of Thrones")  # Título de la ventana
        self.root.geometry("800x600")  # Tamaño de la ventana
        self.root.configure(bg="#0e0e0e")  # Color de fondo
        
        self.personajes: List[Dict] = []  # Lista para almacenar personajes
        self.archivo_json = "personajes_db.json"  # Nombre del archivo de base de datos
        self.raiz: Optional[Nodo] = None  # Raíz del árbol de decisión
        
        # Inicialización de componentes
        self._cargar_recursos()  # Carga imágenes
        self._configurar_estilos()  # Configura estilos visuales
        self._cargar_base_datos()  # Carga la base de datos
        self._configurar_interfaz_principal()  # Configura la interfaz principal

    # Método para cargar recursos visuales (imágenes)
    def _cargar_recursos(self):
        """Carga imágenes y recursos visuales"""
        self.logo_image = self._cargar_imagen("targaryen.png")  # Logo del juego
    ...
    def _cargar_base_datos(self):
        ...
        if not os.path.exists(self.archivo_json):
            self.personajes = []  # Lista vacía si no existe el archivo
            self._guardar_base_datos()  # Crea el archivo
        else:
            try:
                with open(self.archivo_json, 'r', encoding='utf-8') as f:
                    self.personajes = json.load(f)  # Carga los datos del archivo
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo cargar la base de datos: {str(e)}")
                self.personajes = []  # Lista vacía si hay error

    # Método para guardar la base de datos
    def _guardar_base_datos(self):
        """Guarda la base de datos de personajes"""
        with open(self.archivo_json, 'w', encoding='utf-8') as f:
            json.dump(self.personajes, f, ensure_ascii=False, indent=2)  # Guarda con formato
    ...
    def iniciar_juego(self):
        ...
        self._construir_arbol_decision()  # Construye el árbol de decisión
        self._jugar(self.raiz)  # Comienza el juego

    # Método para construir el árbol de decisión
    def _construir_arbol_decision(self):
        """Construye el árbol de decisión"""
        self.raiz = self._construir_nodo(self.personajes.copy())  # Comienza con todos los personajes

    # Método recursivo para construir nodos
    def _construir_nodo(self, personajes: List[Dict], profundidad: int = 0) -> Nodo:
        ...
        # Caso base: solo queda un personaje
        if len(personajes) == 1:
        ...
        # Si no hay buena división, devuelve el personaje más común
        if atributo is None or ganancia < 0.1:
            return Nodo(personaje=self._personaje_mas_comun(personajes))
        
        # Crea un nuevo nodo con el atributo y valor
        nodo = Nodo(atributo=atributo, valor=valor)
        
        # Divide los personajes según la respuesta
        ...
        # Si alguna división está vacía, devuelve el más común
    ...
    # Método para encontrar el mejor atributo para dividir
    def _mejor_atributo(self, personajes: List[Dict]) -> Tuple[Optional[str], Optional[str], float]:
        ...
        # Itera sobre todos los atributos disponibles
        for atributo in self.ATRIBUTOS_DISPONIBLES:
            # Obtiene valores únicos para el atributo
            valores = {str(p["atributos"].get(atributo, "")) for p in personajes if p["atributos"].get(atributo)}
            
            # Si hay menos de 2 valores, no es útil para dividir
            if len(valores) < 2:
                continue
                
            # Evalúa cada valor posible
    ...
    def _calcular_ganancia(self, personajes: List[Dict], atributo: str, valor: str) -> float:
        ...
        peso_si = len(si_personajes)/len(personajes)
        peso_no = len(no_personajes)/len(personajes)
        entropia_posterior = (peso_si * self._calcular_entropia(si_personajes)) + (peso_no * self._calcular_entropia(no_personajes))
        
        # Retorna la ganancia de información
    ...
    def _calcular_entropia(self, personajes: List[Dict]) -> float:
        ...
        # Cuenta la frecuencia de cada personaje
        conteo = {}
        for p in personajes:
            conteo[p["nombre"]] = conteo.get(p["nombre"], 0) + 1
        
        total = len(personajes)
        # Calcula la entropía usando la fórmula -Σ p*log2(p)
        return -sum((count/total) * math.log2(count/total) for count in conteo.values())

    # Método para encontrar el personaje más común
    def _personaje_mas_comun(self, personajes: List[Dict]) -> str:
        """Devuelve el nombre del personaje más común en la lista"""
        conteo = {}
        for p in personajes:
            conteo[p["nombre"]] = conteo.get(p["nombre"], 0) + 1
        return max(conteo.items(), key=lambda x: x[1])[0]  # Personaje con mayor conteo

    # Método para formatear preguntas legibles
    def _formatear_pregunta(self, atributo: str, valor: str) -> str:
        ...
        formatos = {
            ...
            "casa": f"pertenece a la casa {valor}",
            "titulo": f"tiene el título '{valor}'",
            "lugar": f"está asociado a {valor}",
            "muerte": f"murió {valor}"
        }
    ...
    def _jugar(self, nodo: Nodo):
        ...
        if nodo.personaje:  # Si es un nodo hoja (tiene personaje)
            ...
            if respuesta == "Sí":
                self._mostrar_info_personaje(nodo.personaje)  # Muestra información
            else:
                self._agregar_nuevo_personaje(nodo)  # Aprende nuevo personaje
        else:
            # Formatea la pregunta según el atributo
            pregunta = (
                "¿Tu personaje está vivo?" if nodo.atributo == "vivo" else
                f"¿Tu personaje {self._formatear_pregunta(nodo.atributo, nodo.valor)}?"
            )
            
            respuesta = self._mostrar_pregunta("Adivina Quién - Game of Thrones", pregunta, ("Sí", "No"))
            
            # Decide qué nodo seguir según la respuesta
            siguiente_nodo = nodo.si if (respuesta == "Sí" and (nodo.atributo != "vivo" or nodo.valor == "si")) else nodo.no
            self._jugar(siguiente_nodo)  # Llama recursivamente

    # Método para crear ventanas modales
    def _crear_ventana_modal(self, titulo: str, tamaño: str = "400x200") -> Toplevel:
        """Crea una ventana modal con estilo consistente"""
        ventana = Toplevel(self.root)  # Crea una nueva ventana
        ventana.title(titulo)  # Establece título
        ventana.geometry(tamaño)  # Establece tamaño
        ventana.configure(bg="#1a1a1a")  # Color de fondo
        ventana.grab_set()  # Hace la ventana modal
        return ventana

    # Método para mostrar preguntas con opciones
    def _mostrar_pregunta(self, titulo: str, mensaje: str, opciones: Tuple[str]) -> str:
        ...
        ventana = self._crear_ventana_modal(titulo)  # Crea la ventana
        resultado = [None]  # Almacena el resultado
        ...
        self.root.wait_window(ventana)  # Espera hasta que se cierre
        return resultado[0]  # Retorna la opción seleccionada

    # Método para cerrar diálogos y guardar resultado
    def _cerrar_dialogo(self, ventana: Toplevel, resultado: list, valor: str):
        """Cierra un diálogo y guarda el resultado"""
        resultado[0] = valor  # Guarda el valor seleccionado
        ventana.destroy()  # Cierra la ventana

    # Método para mostrar diálogos simples
    ...
    # Método para mostrar información de personaje
    def _mostrar_info_personaje(self, personaje: Dict):
        ...
        # Muestra cada atributo del personaje
    ...
    # Método para mostrar formulario de nuevo personaje
    ...
    # Método para agregar nuevo personaje cuando el sistema falla
    def _agregar_nuevo_personaje(self, nodo: Nodo):
        ...
        def guardar_y_actualizar(nombre: str, atributos: Dict):
            # Pide una pregunta diferenciadora
            ...
            # Pide confirmación de la respuesta esperada
            respuesta = self._mostrar_pregunta(
                "Respuesta",
            ...
            # Reestructura el árbol con la nueva información
        ...
        # Muestra el formulario con la función de guardado personalizada
        self._mostrar_formulario_personaje(
            "Nuevo Personaje",
            lambda n, a: guardar_y_actualizar(n, a) and self._guardar_personaje(n, a, "Aprendido", f"¡He aprendido sobre {n}! ¡Gracias!")
        )

    # Método para mostrar formulario genérico de personaje
    ...
    def _procesar_formulario(self, nombre_var: StringVar, atributos_vars: Dict[str, StringVar], ventana: Toplevel, comando_guardar):
        ...
        nombre = nombre_var.get().strip()  # Obtiene el nombre
        if not nombre:
            self._mostrar_dialogo("Error", "El nombre no puede estar vacío.")
            return
        
        # Obtiene los atributos no vacíos
        atributos = {
            a: v.get().strip() 
        ...
        # Ejecuta el comando de guardado y cierra la ventana si es exitoso
    ...
    def _guardar_personaje(self, nombre: str, atributos: Dict, titulo: str = "Éxito", mensaje: str = None):
        ...
        self._guardar_base_datos()  # Guarda los cambios
...
if __name__ == "__main__":
    ...
    app = AdivinaQuienGoT(root)  # Crea la instancia del juego
